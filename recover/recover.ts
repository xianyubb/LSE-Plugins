...
const PLUGIN_NAME: string = "recover";
...
/** 城市 */
const City: Array<City1> = Conf.init("city", [
    {
        /** 城市名  */
        name: "city1",
        /** 坐标数组 */
 ...
 * @returns 坐标数组
 */
function IntPosToArray(pos: IntPos): Array<number> {
 ...
 * @returns 坐标对象
 */
function ArrayToIntPos(pos: Array<number>): IntPos {
 ...
 * @param player 玩家对象
 */
function getPos(player: Player): Array<number> {
 ...
 * @param player 玩家对象
 */
function getPos2(player: Player): IntPos {
 ...
 */
function isInCity(player: Player): boolean {
    let res: boolean;
...
async function setListener() {
    mc.listen("onPlayerDie", (player: Player, _source: Entity) => {
    ...
    setInterval(() => {
        (async () => {
            mc.getOnlinePlayers().forEach((player: Player) => {
                if (player.gameMode !== 0) return;
                if (!isInCity(player)) return;
 ...
 */
function onCmd(players: Player[], out: CommandOutput) {
    players.forEach((player: Player) => {

        const pos: Array<number> = DataFile.get(player.xuid).DeathPos;
        let time = new Date().getTime();
        DataFile.set(player.xuid, {
            DeathPos: IntPosToArray(player.blockPos),
            DeathTime: time,
    ...
    cmd.setCallback((_cmd, ori, out, res) => {
        switch (ori.type) {
            case OriginType.Player: {
                let player: Player = ori.player;
                if (!player.isOP()) out.error("你没有权限使用此命令");
                else return onCmd(res.player, out);
                break;
            };
            default: {
